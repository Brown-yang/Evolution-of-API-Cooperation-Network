...
# 根据保存的图结构 构建图
txt = []
with open('./new_Network_different_metric/new_BACommunity_edges2000_resolution_1.4.txt', 'r', encoding='utf-8') as f:
    for line in f:
        txt.append(line)
...
model_X2 = Word2Vec.load('./Node2vec_models/Node2Vec_cooperation16_resolution_1.4.model')
...
label = label[:node2vec_features.shape[0]]



cluster2 = KMeans(n_clusters=len(all_label_category), random_state=9).fit(node2vec_features)
pre_label = cluster2.labels_

pre_label_spect = SpectralClustering(n_clusters=len(all_label_category), gamma=0.1).fit_predict(node2vec_features)



minibatchkmeans = MiniBatchKMeans(n_clusters=len(all_label_category), random_state=0, batch_size=6).fit(node2vec_features)
